...
#if USE_DEMO_ECHO
const char* BACKEND_BASE = "https://webhook.site/e390cf37-7ff1-4e11-813b-0b9a2e7480c7";   // your request capture URL
#else
// Backend base URL
...
#ifndef LED_BUILTIN
#define LED_BUILTIN 4    // Default to flash LED on many ESP32-CAM boards. Override with -DLED_BUILTIN=xx
#endif
...
#ifndef BUTTON_PIN
#define BUTTON_PIN 0     // Use BOOT button (GPIO0). Override with -DBUTTON_PIN=xx
#endif

const unsigned long DEBOUNCE_MS = 20;
const unsigned long CLICK_TIMEOUT_MS = 700;   // max gap between clicks for multi-click (easier double/triple)
...
const unsigned long BOOT_INPUT_GRACE_MS = 2000; // ignore button input for first 2s after boot to avoid BOOT hold issues
...
bool httpGet(const String& path, String& bodyOut, int& codeOut) {
  ...
  HTTPClient http;
  String url = String(BACKEND_BASE) + remapGetPath(path);
  Serial.printf("\n➡️ GET %s\n", url.c_str());
...
bool httpPostJson(const String& path, const String& json, String& bodyOut, int& codeOut) {
  ...
  HTTPClient http;
  String url = String(BACKEND_BASE) + remapPostPath(path);
...
// Drive multiple likely LED pins to maximize chance of visibility on various boards
const int LED_PINS[] = { LED_BUILTIN, 4, 33 };
const int LED_PINS_COUNT = sizeof(LED_PINS)/sizeof(LED_PINS[0]);
...
void handleButton() {
  ...
  if (waitingForSecondClick && (now - lastReleaseMs > CLICK_TIMEOUT_MS)) {
    // Interpret clicks
    Serial.printf("Click decision: %d clicks in %lums window\n", clickCount, (unsigned long)CLICK_TIMEOUT_MS);
    if (clickCount >= 3) {
...
void setup() {
  ...
  Serial.printf("Using BOOT button on GPIO%d for input. Do NOT hold during reset. Double-click/triple-click gap: %lums.\n", BUTTON_PIN, (unsigned long)CLICK_TIMEOUT_MS);
  connectWiFi();
...
void loop() {
  ...
  unsigned long now = millis();
  // Ignore BOOT button input for a short period after reset to avoid accidental actions
  static unsigned long bootAt = millis();
  if (millis() - bootAt < BOOT_INPUT_GRACE_MS) {
    // Still perform background tasks, but skip handling button gestures
  } else {
    handleButton();
  }
